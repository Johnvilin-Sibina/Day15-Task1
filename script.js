...
function create_h1(tagname, content) {
  var res = document.createElement(tagname);
  res.innerHTML = content;
  return res;
}

function create_para(tagname,content){
    var res = document.createElement(tagname);
    res.innerHTML = content;
    return res;
}

function create_div(tagname){
    var res = document.createElement(tagname);
    return res;
}
...
function createTable(start, end) {
  clear_table();
  var table = create_table("table", "width", "90%");
  table.className = "table table-bordered";

  var thead = create_thead("thead");
  var th_tr = create_tr("tr"); 
...
div2.id = "buttons";
div2.className = "d-flex justify-content-center align-items-center";
...
div2.append(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s00);
document.body.append(h1,para,div2);
